...
/**
  ...
  */
const firstAccountTags = accounts[0].tags.flatMap(tag => Array.isArray(tag) ? tag : [tag]);

console.log(firstAccountTags);
 /**
  * Exercise 02
  * 
  * Check whether the tags for the first account includes the "consectetur" tag 
  */
 const includesConsectetur = accounts[0].tags.flat().includes("consectetur");

 console.log("The output will be", includesConsectetur);  // Output will be true or false
 /**
  * Exercise 03
  * 
  * Get a list of all the tags for all the accounts and join them together as a string separated by commas.
  */
 const allTags = accounts.flatMap(account => account.tags.flat()).join(', ');
  ...
  */
 const allFriends = accounts.flatMap(account => account.friends.map(friend => friend.name));
  ...
  */
const filteredFriends = accounts
  .filter(account => account.age < 30 && Number(account.balance.replace('$', '').replace(',', '')) > 1500)
  .flatMap(account => account.friends.map(friend => friend.name));
